...
GameScene::GameScene(void) : m_grid("lvl/testLvl.dat", CELL_WIDTH, CELL_HEIGHT) {
	...
	POTATO = { { potato_x, potato_y, 200, 200 }, ObjectID::POTATO };
}
...
void GameScene::Update(void) {
	...
	if ((potato_x < (W.GetHeight() - 180) || potato_x >= (W.GetHeight() - 180)) && (potato_x >(-20))) {
		if (IM.IsKeyHold<KEY_BUTTON_UP>()) {
			potato_y -= TM.GetDeltaTime();
		}
	}
	if (potato_x < (W.GetHeight() - 180)) {
		if (IM.IsKeyHold < KEY_BUTTON_DOWN>()) {
			potato_y += TM.GetDeltaTime();
		}
	}
	if (potato_x < (W.GetWidth()-180)) {
		if (IM.IsKeyHold < KEY_BUTTON_RIGHT>()) {
			potato_x += TM.GetDeltaTime();
		}
	}
	if ((potato_x < (W.GetWidth() - 180) || potato_x >= (W.GetWidth() - 180)) && (potato_x > (-20))) {
